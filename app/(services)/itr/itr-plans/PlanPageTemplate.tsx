...
export default function PlanPage({ plan }: { plan: PlanConfig }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const obs = new IntersectionObserver(([entry]) => setBuyBarShow(!entry.isIntersecting), { threshold: 0 });
    obs.observe(el);
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-8 h-16 flex items-center justify-between gap-4 sm:gap-6">
              ...
              <div className="flex items-center gap-4 min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span
                    className={`text-[10px] font-bold text-white px-2 py-0.5 rounded-md flex-shrink-0 ${tierStyles[plan.id].text.replace("text-", "bg-")} text-white`}
                  >
                    {discount}% OFF
                  </span>
                </div>
            ...
            >
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {plan.servicesIncluded.map((s, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 p-4 rounded-2xl bg-gray-50 border border-gray-100 hover:border-gray-200 transition-colors"
                  >
                    <div
                      className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${tierStyles[plan.id].bgSoft}`}
                    >
            ...
            >
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {plan.whoShouldBuy.map((w, i) => (
                  <div
                    key={i}
                    className={`flex items-start gap-3 p-4 rounded-2xl transition-colors border ${tierStyles[plan.id].bgSoft} ${tierStyles[plan.id].borderLight}`}
                  >
                    <div
                      className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${tierStyles[plan.id].text.replace("text-", "bg-")} text-white`}
                    />
              ...
              {plan.howItsDone.map((s, i) => (
                <div key={i} className="flex gap-5">
                  <div className="flex flex-col items-center w-10 flex-shrink-0">
                    <div
                      className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white z-10"
                      style={{
            ...
            >
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {plan.documentsRequired.map((doc, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 p-4 rounded-2xl bg-white border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all"
                  >
                    <div
                      className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${tierStyles[plan.id].bgSoft}`}
                    >
 ...
 ══════════════════════════════════════════════════ */}
      <div className="bg-white py-16">
        ...
        >
          <div
            className={`absolute -top-16 -right-16 w-56 h-56 rounded-full blur-3xl pointer-events-none opacity-20 ${tierStyles[plan.id].text.replace("text-", "bg-")} text-white`}
          />
          <div
          ...
          <div className="relative px-5 py-10 sm:px-10 sm:py-12 lg:px-14 lg:py-14 flex flex-col sm:flex-row items-center justify-between gap-6 sm:gap-8">
            <div className="text-center sm:text-left">
              ...
              <p className="text-gray-500 text-sm leading-relaxed max-w-md">
                Have questions about which plan suits you? Talk to one of our Chartered Accountants — free of charge,
                zero commitment.
              </p>
            </div>
