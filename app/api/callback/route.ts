...
import { callbackRequestUserEmailTemplate, callbackRequestAdminEmailTemplate } from "@/lib/email/templates";
...
export async function POST(req: Request) {
  try {
    ...
    const adminEmailHtml = callbackRequestAdminEmailTemplate(name, email, phone, service);
    ...
    sendEmail({
      ...
      html: adminEmailHtml,
    }).catch((err) => console.error("Failed to send admin callback notification:", err));
    ...
    return NextResponse.json({ success: false, message: "Failed to submit callback request" }, { status: 500 });
  }
}
