...
async function fetchFeed(feedUrl: string, label: string, tag: string, maxItems = 2): Promise<LiveNewsItem[]> {
  try {
    const res = await fetch(feedUrl, {
      next: { revalidate: 1800 },
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; LawizerBot/1.0; +https://lawizer.com)",
      },
...
export async function GET() {
  ...
  const results = await Promise.allSettled(RSS_FEEDS.map((f) => fetchFeed(f.url, f.label, f.tag, 2)));
