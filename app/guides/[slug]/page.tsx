...
export async function generateMetadata(props: {
  ...
  const category = guide.categories?.nodes?.find(
    (c) => c.slug !== 'guides'
  )?.name ?? 'Legal Guide';
...
export default async function GuideDetailPage(props: {
  ...
  const isGuide = guide.categories?.nodes?.some(
    (c) => c.slug === 'guides' || c.name.toLowerCase() === 'guides'
  );
  if (!isGuide) notFound();
  ...
  const readTime = guide.content
    ? computeReadingTime(guide.content)
    : null;
  ...
  const serviceLabel =
    guide.guideFields?.serviceLabel ??
    'Need help with this? Lawizer handles it end-to-end.';
    ...
    <>
      ...
      <article className="bg-[#f4f5f7] min-h-screen pb-24">
        ...
        <header className="max-w-5xl mx-auto px-5 pt-12 pb-6">
          {/* Breadcrumb */}
          <nav
            className="text-sm text-slate-500 mb-5 flex items-center gap-2 flex-wrap"
            aria-label="Breadcrumb"
          >
            <Link
              href="/guides"
              className="hover:text-[#e94560] transition-colors font-medium"
            >
              Guides
            </Link>
            {category && (
            ...
            <span className="text-slate-300">›</span>
            <span className="text-slate-400 line-clamp-1 max-w-[280px]">
              {guide.title}
            </span>
          </nav>
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            {publishedDate && (
              <span className="text-slate-400 text-xs">· {publishedDate}</span>
            )}
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0f0c29] via-[#302b63] to-[#24243e] p-8 md:p-10">
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
                <div>
                  ...
                  <h2 className="text-white text-xl md:text-2xl font-extrabold leading-snug">
                    {serviceLabel}
                  </h2>
                  <p className="text-white/60 text-sm mt-1">
                    Expert legal team · Fixed fees · 100% online
                  </p>
                </div>
                <Link
                  href={serviceLink}
