...
import { X, Mail, Lock, Loader2, ArrowRight, User, Phone, Briefcase, CheckCircle } from "lucide-react";
...
export function ProfessionalModal({ onClose, onClientLoginRedirect, onForgotPassword }: ProfessionalModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-[9999] flex items-center justify-center p-4 font-sans animate-[fadeIn_0.2s_ease]">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 sm:p-8 relative border border-slate-200">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 transition p-1 rounded-full hover:bg-slate-100 cursor-pointer"
        ...
        <form onSubmit={isSignUp ? handleSignUp : handleLogin} className="space-y-3.5">
          ...
          {isSignUp && (
            <>
              <div>
                <label className="text-[11px] font-bold text-gray-700 uppercase tracking-wider block mb-1">Full Name</label>
              ...
              <div>
                <label className="text-[11px] font-bold text-gray-700 uppercase tracking-wider block mb-1">Phone Number</label>
                <div className="relative">
              ...
              <div>
                <label className="text-[11px] font-bold text-gray-700 uppercase tracking-wider block mb-1">Specialization</label>
                <div className="relative">
          ...
          {/* Email */}
          <div>
            <label className="text-[11px] font-bold text-gray-700 uppercase tracking-wider block mb-1">Professional Email</label>
            <div className="relative">
          ...
          {/* Password */}
          <div>
            <label className="text-[11px] font-bold text-gray-700 uppercase tracking-wider block mb-1">Password</label>
            <div className="relative">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2.5 flex items-center justify-center bg-[#c92c41] text-white text-xs font-bold rounded-xl hover:bg-[#b32538] transition disabled:opacity-70 cursor-pointer shadow-md active:scale-95 mt-2"
          >
            {loading ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
              <>
                <span>{isSignUp ? "Register Account" : "Sign In to Expert Portal"}</span>
        ...
        <div className="mt-5 pt-3 border-t border-gray-100 text-center text-xs text-gray-500">
          <span>Client looking for legal help? </span>
          <button
            type="button"
            onClick={() => {
              onClose();
              if (onClientLoginRedirect) onClientLoginRedirect();
            }}
            className="text-[#c92c41] font-bold hover:underline cursor-pointer"
          >
            Client Login
          </button>
        </div>
