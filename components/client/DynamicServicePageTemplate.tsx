...
function DynamicHeroWithAddons({ data }: { data: any }) {
  ...
  const { title, subtitle, price, originalPrice, addons, theme, icon } = data;

  // Resolve floating icon from lucide mapped icons
  ...
  const FloatingIcon = iconMap[icon] || Shield; // Default to Shield if not found

  return (
    <section
      ref={heroRef}
      className={`relative min-h-screen flex flex-col items-center justify-center text-center overflow-hidden ${theme?.heroBg || "bg-gradient-to-br from-[#1e1b4b] via-[#6d28d9] to-[#2e1065]"} text-white px-4 sm:px-6 pt-[100px] pb-[56px]`}
    >
      {/* Glow Orbs */}
      <div
        className={`absolute top-1/4 left-1/4 w-72 h-72 ${theme?.orb1 || "bg-purple-500/20"} blur-3xl rounded-full pointer-events-none`}
      />
      <div
        className={`absolute bottom-1/4 right-1/4 w-72 h-72 ${theme?.orb2 || "bg-violet-500/20"} blur-3xl rounded-full pointer-events-none`}
      />
