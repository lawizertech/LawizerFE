      ...
      <section id="services" className="bg-white pt-24 pb-20 relative overflow-hidden">
        ...
        <div className="max-w-[1180px] mx-auto px-6 relative z-10">
          ...
          >
            ...
            <h2 className="font-[family-name:var(--)] text-[clamp(28px,3.8vw,50px)] font-extrabold text-[#0D0F14] leading-[1.1] tracking-[-0.025em] m-0 mb-4">
              Every legal need.
              <br />
            ...
            {PHASES.map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* phase badge */}
                  <span
                    className={`inline-flex items-center gap-1.5 w-fit rounded-full px-3 py-1 text-[11px] font-bold mb-3 transition-all duration-220 ${on ? p.tw.bg : "bg-[#EAEDF2]"} ${on ? "text-white" : "text-[#8A90A2]"}`}
                  >
                    <span
                      className={`w-4 h-4 rounded-full inline-flex items-center justify-center text-[9px] font-black leading-none ${on ? "bg-white/25 text-white" : "bg-[#C4C9D6] text-[#6B7280]"}`}
                    >
                      {p.id}
                    </span>
                    Phase {p.id}
                  ...
                  <span
                    className={`font-[family-name:var(--,'Syne',sans-serif)] text-[22px] font-extrabold leading-[1.15] mb-2 transition-colors duration-220 ${on ? p.tw.text : "text-[#0D0F14]"}`}
                  >
                    {p.label}
                  </span>
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 mt-3.5 text-xs font-bold transition-colors duration-220 ${on ? p.tw.text : "text-[#B0B7C3]"}`}
                  >
                    {total} services
                    <span
                      className={`inline-block transition-transform duration-220 ${on ? "translate-x-[3px]" : "translate-x-0"}`}
                    >
                      →
                    </span>
                  </span>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {/* panel header */}
              <div
                className={`flex items-center justify-between flex-wrap gap-3.5 px-8 py-[22px] border-b-[1.5px] border-[#EEF1F6] ${phase.tw.bgSoft}`}
              >
                <div className="flex items-center gap-3.5">
                  <div
                    className={`w-12 h-12 rounded-[14px] flex items-center justify-center text-[22px] shrink-0 ${phase.tw.bg}`}
                  >
                    {phase.id === 1 ? "🏗️" : phase.id === 2 ? "🛡️" : "♻️"}
                  </div>
                  <div>
                    <div className="font-[family-name:var(--,'Syne',sans-serif)] text-lg font-extrabold text-[#0D0F14]">
                      Phase {phase.id} — {phase.label}
                    </div>
                    <div className="text-[13px] text-gray-500 mt-[3px]">
                      {phase.categories.reduce((n, c) => n + c.items.length, 0)} services across{" "}
                ...
                {phase.categories.map((cat, ci) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 mb-3.5">
                      <span
                        className={`text-[10.5px] font-extrabold uppercase tracking-[1.8px] px-3.5 py-1.5 rounded-full shrink-0 ${phase.tw.text} ${phase.tw.bgSoft}`}
                      >
                        {cat.label}
                      </span>
              ...
              {/* CTA strip */}
              <div
                className={`mx-4 sm:mx-6 mb-4 sm:mb-6 p-5 sm:px-6 rounded-2xl border-[1.5px] flex items-center flex-wrap gap-4 justify-between ${phase.tw.bgSoft} ${phase.tw.border}/20`}
              >
  ...
  return (
    ...
    >
      ...
      <span className="flex flex-col items-end shrink-0 ml-1">
        <span className="text-[11px] text-[#B0B7C3] line-through leading-none">{svc.was}</span>
        <span
          className={`text-sm font-extrabold leading-[1.2] transition-colors duration-150 ${hov ? phase.tw.text : "text-[#0D0F14]"}`}
        >
          {svc.price}
        </span>
      </span>
